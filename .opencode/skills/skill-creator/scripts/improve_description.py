...
by calling `opencode2 run` as a subprocess (OpenCode port of the original
`claude -p` approach — uses the session's configured provider auth, so no
separate ANTHROPIC_API_KEY is needed).
"""
...
def _extract_text_parts(ndjson: str) -> str:
    """Concatenate the text parts from `opencode2 run --format json` output.

    The pretty `default` output wraps the response in ANSI styling and a
    session header; the JSON event stream is clean and machine-readable.
    """
    parts = []
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 900) -> str:
    """Run `opencode2 run` with the prompt on stdin and return the text response.

    Prompt goes over stdin (message `-`) because it embeds the full SKILL.md
    body and can easily exceed the Windows command-line length limit.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "opencode2 CLI not found; install OpenCode V2 to run description optimization"
        ) from None
    if result.returncode != 0:
        raise RuntimeError(
            f"opencode2 run exited {result.returncode}\nstderr: {result.stderr}"
        )
    return _extract_text_parts(result.stdout)
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; CLI subprocesses are one-shot,
