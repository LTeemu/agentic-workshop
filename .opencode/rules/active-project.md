...
2. **Workshop infrastructure** — If working on the workshop dashboard, sandbox, or opencode configuration itself, scope is the **workspace root** (ignore `.active-project`).
3. **`.active-project`** — Otherwise, read it at the workspace root. It contains a path like `projects/project-name`. Scope all file operations there.
