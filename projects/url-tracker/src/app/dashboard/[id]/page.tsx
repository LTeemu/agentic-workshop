  ...
  if (isError) {
    return (
      <div
        ...
        style={{ border: '1px solid var(--color-error)', background: 'rgba(239,68,68,0.06)' }}
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:flex-wrap">
          ...
          <select
            ...
            onChange={(e) =>
              updatePage.mutate({
                pageId: page.id,
                scrapeInterval: e.target.value as 'manual' | 'hourly' | 'daily',
              })
            }
            style={selectStyle}
          ...
          >
            {scrapePage.isPending ? (
              ...
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                <path
                  d="M11 6.5a4.5 4.5 0 11-4.5-4.5M11 2v3H8"
                  stroke="currentColor"
                  strokeWidth="1.4"
                  strokeLinecap="round"
                  strokeLinejoin="round"
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-4">
        ...
        <select
          value={attribute}
          onChange={(e) => setAttribute(e.target.value as 'text' | 'href' | 'src')}
          className="rounded-md px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1"
          style={{ ...inputStyle, '--tw-ring-color': 'var(--color-accent)' } as React.CSSProperties}
        >
          <option value="text" style={{ background: 'var(--color-surface)' }}>
            Text
          </option>
          <option value="href" style={{ background: 'var(--color-surface)' }}>
            Link
          </option>
          <option value="src" style={{ background: 'var(--color-surface)' }}>
            Image
          </option>
        </select>
        <select
          value={valueType}
          onChange={(e) => setValueType(e.target.value as 'text' | 'number' | 'boolean')}
          className="rounded-md px-2.5 py-1.5 text-sm focus:outline-none focus:ring-1"
          style={{ ...inputStyle, '--tw-ring-color': 'var(--color-accent)' } as React.CSSProperties}
        >
          <option value="number" style={{ background: 'var(--color-surface)' }}>
            Number
          </option>
          <option value="text" style={{ background: 'var(--color-surface)' }}>
            Text
          </option>
          <option value="boolean" style={{ background: 'var(--color-surface)' }}>
            Boolean
          </option>
        </select>
      </div>
      <button
        type="submit"
        disabled={addField.isPending || !label.trim() || !cssSelector.trim()}
        style={{
...
function NumericFieldCard({ field, scrapeError }: { field: Field; scrapeError?: string }) {
  ...
  return (
    ...
    >
        ...
        {editing ? (
          <div className="flex flex-1 flex-wrap items-center gap-2">
            <input
              ...
              style={
                { ...inputStyle, '--tw-ring-color': 'var(--color-accent)' } as React.CSSProperties
              }
            />
            <input
              type="text"
            ...
            <button
              ...
              style={{ color: 'var(--color-accent)' }}
              className="cursor-pointer rounded-md px-2 py-1 text-xs font-semibold disabled:opacity-40 hover:opacity-80 transition-all"
            >
              Save
            </button>
            <button
              ...
              style={{ color: 'var(--color-text-tertiary)' }}
              className="cursor-pointer rounded-md px-2 py-1 text-xs hover:opacity-80 transition-all"
            >
              Cancel
            </button>
          </div>
          ...
          <>
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span
                className="data-value shrink-0 text-[10px]"
                style={{ color: 'var(--color-text-tertiary)' }}
              >
                {field.cssSelector}
              </span>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              {changeDir === 'up' && (
                <span
                  ...
                  style={{ color: 'var(--color-success)', background: 'rgba(34,197,94,0.12)' }}
                >
                  ↑
                </span>
              )}
              ...
              {field.notifyOnChange && (
                <span className="chip text-[10px]" style={{ color: 'var(--color-accent)' }}>
                  🔔
                </span>
              )}
              <button
                onClick={() => {
                  ...
                  setEditSelector(field.cssSelector);
                  setEditAttr(field.attribute);
                  setEditType(field.valueType);
                  setEditNotify(field.notifyOnChange);
                  setEditAlertMin(field.alertMin ?? '');
                  setEditAlertMax(field.alertMax ?? '');
                  setEditing(true);
                }}
      ...
      {editing && (
        <div
          className="mb-3 flex flex-wrap items-center gap-3 rounded-lg p-2.5"
        ...
        >
          ...
          >
            ...
            <input
              type="text"
              value={editAlertMin}
              onChange={(e) => setEditAlertMin(e.target.value)}
              placeholder="—"
              className="w-16 rounded-md px-1.5 py-0.5 text-xs font-mono focus:outline-none focus:ring-1"
              style={
                { ...inputStyle, '--tw-ring-color': 'var(--color-accent)' } as React.CSSProperties
              }
            />
          ...
          >
            ...
            <input
              type="text"
              value={editAlertMax}
              onChange={(e) => setEditAlertMax(e.target.value)}
              placeholder="—"
              className="w-16 rounded-md px-1.5 py-0.5 text-xs font-mono focus:outline-none focus:ring-1"
              style={
                { ...inputStyle, '--tw-ring-color': 'var(--color-accent)' } as React.CSSProperties
      ...
      <div className="mb-4 grid grid-cols-5 gap-3">
        <div>
          ...
          <div
            className="mt-0.5 text-lg font-semibold data-value"
            style={{ color: 'var(--color-text-primary)' }}
          >
            {current?.toFixed(2) ?? '—'}
          </div>
        ...
        <div>
          ...
          <div
            className="mt-0.5 text-sm data-value"
            style={{ color: 'var(--color-text-secondary)' }}
          >
            {previous?.toFixed(2) ?? '—'}
          </div>
              ...
              <tbody>
                {history.map((h) => (
                  <tr key={h.id} style={{ borderTop: '1px solid var(--color-border)' }}>
                    <td
                      className="px-3 py-1.5 data-value font-medium"
                      style={{ color: 'var(--color-text-primary)' }}
                    >
                      {h.value}
                    </td>
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              {field.valueType === 'boolean' && (
                <span
                  ...
                  style={{
                    background: isTruthy ? 'var(--color-success)' : 'var(--color-text-tertiary)',
                  }}
                />
              )}
              {hasChanged && (
      ...
      {latest !== null && previous !== null && (
        <div className="mb-3 grid grid-cols-2 gap-2">
          <div className="rounded-lg p-2.5" style={{ background: 'rgba(255,255,255,0.02)' }}>
            ...
            <div
              className="mt-0.5 break-words text-sm data-value"
              style={{ color: 'var(--color-text-secondary)' }}
            >
              {previous}
            </div>
          ...
          <div
            ...
            style={{
              border: hasChanged ? '1px solid var(--color-accent)' : 'none',
              background: hasChanged ? 'rgba(245,158,11,0.06)' : 'rgba(255,255,255,0.02)',
            }}
          >
            <div
              className="text-[10px] uppercase tracking-wider"
              style={{ color: hasChanged ? 'var(--color-accent)' : 'var(--color-text-tertiary)' }}
            >
              Current {hasChanged && '✦'}
            </div>
            <div
              className="mt-0.5 break-words text-sm font-medium data-value"
              style={{ color: hasChanged ? 'var(--color-accent)' : 'var(--color-text-primary)' }}
            >
              {latest}
            </div>
          </div>
      ...
      {changes.length > 0 ? (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody>
                {changes.map((h) => (
                  <tr key={h.id} style={{ borderTop: '1px solid var(--color-border)' }}>
                    <td
                      className="px-3 py-1.5 break-all data-value font-medium"
                      style={{ color: 'var(--color-text-primary)' }}
                    >
                      {h.value}
                    </td>
        ...
        <div
          className="flex h-16 items-center justify-center rounded-lg text-sm"
          style={{ background: 'rgba(255,255,255,0.02)', color: 'var(--color-text-tertiary)' }}
        >
          No data yet — click &quot;Scrape Now&quot; to start tracking
        </div>
