    ...
    <div className="flex min-h-screen items-center justify-center" style={{ background: 'var(--color-bg)' }}>
      <form onSubmit={handleSubmit} className="flex w-full max-w-sm flex-col gap-5 p-6" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)' }}>
        ...
        <div className="text-center">
          <h1 className="text-lg font-semibold" style={{ color: 'var(--color-text-primary)' }}>Create Account</h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--color-text-tertiary)' }}>Start tracking what matters</p>
        </div>

        {error && <p className="text-sm" style={{ color: 'var(--color-error)' }}>{error}</p>}

        <div className="flex flex-col gap-1.5">
          <label htmlFor="name" className="text-xs font-medium uppercase tracking-wider" style={{ color: 'var(--color-text-secondary)' }}>Name</label>
          <input
            id="name"
            type="text"
            value={name}
        ...
        <p className="text-center text-sm" style={{ color: 'var(--color-text-tertiary)' }}>
          ...
          <Link href="/login" className="hover:underline" style={{ color: 'var(--color-accent)' }}>Sign In</Link>
        </p>
      </form>
    </div>
  );
