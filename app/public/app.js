...
function applyDeviceView() {
  ...
  for (const btn of devicePresets.querySelectorAll('.device-preset')) {
    ...
    if (!preset) continue;
    // Non-active presets show their portrait ratio; the active one follows
    // the current orientation so the tooltip always matches what is shown.
  ...
  if (presetMode) {
    ...
    deviceWidthInput.disabled = true;
    deviceHeightInput.disabled = true;
    // Hovering the disabled inputs (which pass pointer events through to the
    // span) explains the lock and points at Custom.
    const label = mode[0].toUpperCase() + mode.slice(1);
    deviceSize.title = `${label} is locked to ${ratioLabel(base.width, base.height)} aspect ratio — use Custom to change`;
...
function setDeviceMode(mode) {
  if (mode === 'fit') {
    deviceState.mode = 'fit';
  } else if (mode === 'custom') {
    // Start near-full so the frame almost fills the pane; the slack keeps
    // the resize handles easy to grab, and stays free-resizable from there.
    const rect = frameContainer.getBoundingClientRect();
...
function commitDeviceInput() {
  if (deviceState.mode !== 'custom') return; // presets lock their ratio
  const width = Number(deviceWidthInput.value);
  const height = Number(deviceHeightInput.value);
  deviceState.width = clampDimension(width);
  deviceState.height = clampDimension(height);
  applyDeviceView();
  persistDeviceState();
}
...
// Drag handles: custom drags resize one axis (edge) or both (corner); preset
// drags scale the locked ratio between 100% and MIN_FILL of the fill axis.
// Mirrors the log-panel drag pattern (pointer capture, document moves).
let deviceDrag = null;
...
// A cancelled gesture (e.g. touch scroll takeover) must not keep the saved
// size stale or leave a resize cursor stuck on the page.
document.addEventListener('pointercancel', endDeviceDrag);

deviceHandles.addEventListener('pointerdown', startDeviceDrag);

// Presets fill the pane, so re-fit on any pane change (window resize, sidebar
// collapse, log panel drag) with the same debounced observer as the terminal.
window.ResizeUtils.watchResize(frameContainer, applyDeviceView, 100);

applyDeviceView();

// All saved-state restores have run; allow transitions again for user actions.
